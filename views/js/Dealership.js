...
// This function either turns on or off the row highlighting for premium
// items (depending on the value of bShowPremium)
function highlightpremium(idTable, bShowPremium) {
    // if bShowPremium is true, then we're highlighting premium
    ...
    // "premium" attribute on it.
    for (i = 0; i < aTRs.length; i++) {
        if (aTRs[i].getAttribute('premium') && aTRs[i].getAttribute('premium') == "true") {
            if (bShowPremium) {
...
window.addEventListener("load", function () {
    ...
    document.querySelector("#showPremium").addEventListener("click", function () {
        highlightpremium('menuTable', this.checked);
    });
});